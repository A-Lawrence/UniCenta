...
// JG 27 Apr 2014 - missing bracket enclosures                } else if((sCode.length() == 13) && sCode.startsWith("2") || sCode.startsWith("02")) {
            ...
            if (cTrans == '\u007f') {
                ...
                if (!priceWith00) {
                    m_jPrice.setText(m_jPrice.getText() + Character.toString(cTrans));
                } else {
                 ...
                 * **********************************************
                 */
                // Create the reasons for the drop down
                String[] additionPossibilities = new String[]{
                    "GEN_GENCRAFT", "GEN_SEWING", "GEN_KNIT", "GEN_MAT",
                    "GEN_CPSTOCK", "GEN_WORKS", "GEN_DOLLSH", "GEN_FANDB", "GEN_JEWEL",
                    "GEN_MAGA", "GEN_OTHER"
                };
                String[] arrPoss = new String[additionPossibilities.length];
                int pos = 0;
                try {
                    for (String s : additionPossibilities) {
                        ProductInfoExt p = dlSales.getProductInfoByCode(s);
                        arrPoss[pos] = Integer.toString(pos + 1) + " - " + p.getName();
                        pos++;
                    }
                } catch (Exception e) {
                    stateToZero();
                ...
                // Since it's a positive value, we need a reason!
                String rsn = "Please Choose...";
                while (rsn != null && rsn.contains("Please Choose...")) {
                    rsn = (String) JOptionPane.showInputDialog(
                            this,
                            "Select reason for manual price:\n",
                            "Manual Pricing :: Reason",
                ...
                // If no reason is given, we don't continue!
                if (rsn == null || rsn.length() <= 0) {
                    stateToZero();
                    return;
                }

                // Since we've got a reason, let's use that product.
                try {
                    double tmpPrice = product.getPriceSell();
                    String[] optionChosenLine = rsn.split(" - ");
                    product = dlSales.getProductInfoByCode(additionPossibilities[Integer.parseInt(optionChosenLine[0]) - 1]);
                    product.setPriceSell(tmpPrice);
                } catch (Exception e) {
                    stateToZero();
                    return;
                }
                ...
                // Create the reasons for the drop down
                String[] additionPossibilities = new String[]{
                    "GEN_MANAGER", "GEN_PROMOTION", "GEN_PRICECORRECT"
                };
                String[] arrPoss = new String[additionPossibilities.length];
                int pos = 0;
                try {
